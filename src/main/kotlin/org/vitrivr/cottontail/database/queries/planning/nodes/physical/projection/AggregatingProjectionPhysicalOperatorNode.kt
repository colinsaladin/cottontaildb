...
class AggregatingProjectionPhysicalOperatorNode(input: Physical? = null, type: Projection, val fields: List<Name.ColumnName>) : AbstractProjectionPhysicalOperatorNode(input, type) {
    ...
    override val columns: List<ColumnPair>
        get() = this.fields.map {
            val col = this.input?.columns?.find { c -> c.logical().name == it } ?: throw QueryException.QueryBindException("Column with name $it could not be found on input.")
            if (!col.logical().type.numeric) throw QueryException.QueryBindException("Projection of type ${this.type} can only be applied to numeric column, which $col isn't.")
            col
        }
    ...
    override val requires: List<ColumnDef<*>>
        get() = this.fields.map {
            this.input?.columns?.find { c -> c.logical().name == it }?.logical() ?: throw QueryException.QueryBindException("Column with name $it could not be found on input.")
        }
