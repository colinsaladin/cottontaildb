...
class CountProjectionPhysicalOperatorNode(input: Physical? = null, val alias: Name.ColumnName? = null) : AbstractProjectionPhysicalOperatorNode(input, Projection.COUNT) {
    ...
    override val columns: List<ColumnPair>
        get() {
            val name = this.alias ?: (this.input?.columns?.first()?.logical()?.name?.entity()?.column(Projection.COUNT.label()) ?: Name.ColumnName(Projection.COUNT.label()))
            return listOf(ColumnDef(name, Type.Long, false) to null)
        }
